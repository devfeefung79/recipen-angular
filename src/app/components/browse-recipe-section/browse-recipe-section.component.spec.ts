...
import { of } from 'rxjs';
import { mockAreaList, mockAreaOptions, mockCategoryList, mockCategoryOptions, mockCategoryOptionsLong } from '../../mock/data';
...
describe('BrowseRecipeSectionComponent', () => {
  ...
  it('should render the arrow left amd the arrow right icon', () => {
    const compiled = fixture.nativeElement;
    const icons = compiled.querySelectorAll('.browse-option-scroll-container mat-icon');
    expect(icons.length).toBe(2);
  })

  it('should call the correct function when the arrow left icon is clicked', () => {
    const compiled = fixture.nativeElement;
    const buttons = compiled.querySelectorAll('.browse-option-scroll-container button');
